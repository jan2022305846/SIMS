...
<head>
    <meta charset="utf-8">
    <title>Monthly Summary Report - {{ $month }}</title>
    <style>
...
<body>
    <div class="header">
        ...
        <div class="date-range">
            Period: {{ $startDate->format('F Y') }} ({{ $startDate->format('M d, Y') }} - {{ $endDate->format('M d, Y') }})
        </div>
    ...
    <div class="stats">
        ...
        <div class="stat-box">
            <div class="stat-number">{{ number_format($monthlyData['low_stock_items']->count()) }}</div>
            <div class="stat-label">Low Stock Items</div>
        </div>
        <div class="stat-box">
            <div class="stat-number">{{ number_format($monthlyData['inventory_changes']->count()) }}</div>
    ...
    @php
        ...
        $itemSummary = [];
        foreach ($monthlyData['requests'] as $request) {
            ...
            if (!isset($itemSummary[$itemId])) {
                $itemSummary[$itemId] = [
                    ...
                    'stock_additions' => 0, // Stock added during the month
                ];
            }
        ...
        // Calculate stock additions during the month from ActivityLog
        ...
        foreach ($stockAdditionLogs as $log) {
            $changes = $log->changes(); // Use the changes() method instead of properties()
            if (isset($changes['quantity'])) {
                $oldQuantity = $changes['quantity']['old'] ?? 0;
                $newQuantity = $changes['quantity']['new'] ?? 0;
    ...
    @if(count($itemSummary) > 0)
        @foreach($itemSummary as $summary)
            <div class="item-row">
                ...
                <div class="item-details">
                    ...
                    <div class="detail-row">
                        <div class="detail-cell detail-label">Monthly Stock (Beginning):</div>
        ...
        @endforeach
    @else
        <p>No item movement data available for the selected month.</p>
    @endif

    <!-- Weekly Breakdown -->
    <div class="section-title">Weekly Request Breakdown</div>
    @if(count($analytics['weekly_breakdown']) > 0)
        <table>
            <thead>
                <tr>
                    <th>Week</th>
            ...
            <tbody>
                @foreach($analytics['weekly_breakdown'] as $week => $data)
                    <tr>
                        <td>{{ $week }}</td>
                        <td>{{ $data['requests'] }}</td>
    ...
    @if($monthlyData['low_stock_items']->count() > 0)
        <table>
            <thead>
                <tr>
                    <th>Item Name</th>
            ...
            <tbody>
                @foreach($monthlyData['low_stock_items'] as $item)
                    <tr>
                        <td>{{ $item->name }}</td>
