...
<head>
    <meta charset="utf-8">
    <title>Quarterly Summary Report - Q{{$startDate->quarter}} {{$startDate->year}}</title>
    <style>
...
<body>
    <div class="header">
        ...
        <div class="date-range">
            Period: Q{{$startDate->quarter}} {{$startDate->year}} ({{$startDate->format('M d, Y')}} - {{$endDate->format('M d, Y')}})
        </div>
    ...
    <div class="stats">
        ...
        <div class="stat-box">
            <div class="stat-number">{{ number_format($quarterlyData['low_stock_items']->count()) }}</div>
            <div class="stat-label">Low Stock Items</div>
        </div>
        <div class="stat-box">
            <div class="stat-number">{{ number_format($quarterlyData['inventory_changes']->count()) }}</div>
    ...
    @php
        ...
        $itemSummary = [];
        foreach ($quarterlyData['requests'] as $request) {
            ...
            if (!isset($itemSummary[$itemId])) {
                $itemSummary[$itemId] = [
                    ...
                    'stock_additions' => 0, // Stock added during the quarter
                ];
            }
        ...
        // Calculate stock additions during the quarter from ActivityLog
    ...
    @if(count($itemSummary) > 0)
        @foreach($itemSummary as $summary)
            <div class="item-row">
                ...
                <div class="item-details">
                    ...
                    <div class="detail-row">
                        <div class="detail-cell detail-label">Quarterly Stock (Beginning):</div>
        ...
        @endforeach
    @else
        <p>No item movement data available for the selected quarter.</p>
    @endif

    <!-- Monthly Breakdown -->
    <div class="section-title">Monthly Request Breakdown</div>
    @if(count($analytics['monthly_breakdown']) > 0)
        <table>
            <thead>
                <tr>
                    <th>Month</th>
            ...
            <tbody>
                @foreach($analytics['monthly_breakdown'] as $month => $data)
                    <tr>
                        <td>{{ $month }}</td>
                        <td>{{ $data['requests'] }}</td>
    ...
    @if($quarterlyData['low_stock_items']->count() > 0)
        <table>
            <thead>
                <tr>
                    <th>Item Name</th>
            ...
            <tbody>
                @foreach($quarterlyData['low_stock_items'] as $item)
                    <tr>
                        <td>{{ $item->name }}</td>
